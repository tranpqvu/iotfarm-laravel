...
(function($, window, undefined) {
    ...
    */
            ...
            if (actionItems.length > 0) {
                ...
                if (this.options.ajax) {
                    var refreshIcon = tpl.icon.resolve(getParams.call(this, { iconCss: css.iconRefresh })),
                        refresh = $(tpl.actionButton.resolve(getParams.call(this, { content: refreshIcon, text: this.options.labels.refresh })))
                        .on("click" + namespace, function(e) {
                            // todo: prevent multiple fast clicks (fast click detection)
                            e.stopPropagation();
                            that.current = 1;
                            /*---- ducldm add on 170905 -----*/
                            that.searchPhrase = "";
                            var txt_search = $(".actionBar .search :input.search-field[type=text]");
        ...
        if (this.options.columnSelection && this.columns.length > 1) {
            ...
            $.each(this.columns, function(i, column) {
                if (column.visibleInSelection) {
                    var item = $(tpl.actionDropDownCheckboxItem.resolve(getParams.call(that, { name: column.id, label: column.text, checked: column.visible })))
    ...
    function renderRowCountSelection(actions) {
        ...
        if ($.isArray(rowCountList)) {
            ...
            $.each(rowCountList, function(index, value) {
                var item = $(tpl.actionDropDownItem.resolve(getParams.call(that, { text: getText(value), action: value })))
        ...
        if (rows.length > 0) {
            ...
            $.each(rows, function(index, row) {
                ...
                if (that.selection) {
                    ...
                    cells += tpl.cell.resolve(getParams.call(that, { content: '<input name="select" type="checkbox" class="select-box" value="' + row[that.identifier] + '">', css: css.selectCell }));
                ...
                $.each(that.columns, function(j, column) {
                    if (column.visible) {
                        ...
                        cells += tpl.cell.resolve(getParams.call(that, {
                            ...
                            //style: (column.width == null) ? "min-width:400px" : "width:" + column.width + ";" }));
                            style: (column.style.length > 0) ? column.style : ""
                        }));
                    }
                });
    ...
    function renderButtonsField() {
        ...
        if (this.options.buttons != undefined) {
            if (this.options.buttons.length > 0) {
                buttons += '<div class ="buttons-list form-group col-lg-3 col-md-3 col-sm-3 col-xs-12" style="float:left; display:block;">';
              
                $.each(this.options.buttons, function(ind, val) {
                    if(val.css!=undefined){
                        button_css = ' style="' + val.css + '" ';
                    }else
                    {
                        button_css='';
                    }
                    ...
                    if (val.name != undefined) {
                        if (val.bclass != undefined) {
                            if (val.onpress != undefined) {
                                buttons += '&nbsp<button type="button" class="new-bootgrid-button ' + val.bclass + ' btn" onclick="' + val.onpress + '()" '+ button_css +' >' + val.name + '</button>';
                            } else {
                                buttons += '&nbsp<button type="button" class="new-bootgrid-button btn ' + val.bclass + '" ' + button_css +' >' + val.name + '</button>';
                            }
                        } else {
                            buttons += '&nbsp<button type="button" class="new-bootgrid-button btn "  ' + button_css + ' >' + val.name + '</button>';
                        }
                    }
        ...
        // console.log("-----render buttons action -----");
        // console.log(buttons);
        $(".bootgrid-header .actionBar .search").before(buttons);
    }
    ...
    //     if (this.options.buttons != undefined) {
    //         if (this.options.buttons.length > 0) {
    //             buttons += '<div class ="buttons-list btn-group col-lg-3 col-md-3 col-sm-3 col-xs-12" style="float:left; display:block;">';
    //             $.each(this.options.buttons, function(ind, val) {
    //                 if (val.name != undefined) {
    //                     if (val.bclass != undefined) {
    //                         if (val.onpress != undefined) {
    //                             buttons += '<button type="button" class="new-bootgrid-button ' + val.bclass + ' btn btn-default" onclick="' + val.onpress + '()">' + val.name + '</button>';
    //                         } else {
    //                             buttons += '<button type="button" class="new-bootgrid-button btn btn-default ' + val.bclass + '">' + val.name + '</button>';
    //                         }
    //                     } else {
    //                         buttons += '<button type="button" class="new-bootgrid-button btn btn-default">' + val.name + '</button>';
    //                     }
    //                 }
    ...
    function renderTableHeader() {
        ...
        //console.log("----- duc = " + this.selection + " | " + this.options.multiSelect);
        if (this.selection) {
            //var selectBox = (this.options.multiSelect) ?
            //    tpl.select.resolve(getParams.call(that, { type: "checkbox", value: "all" })) : "";
            var selectBox = (this.options.multiSelect) ? '<input name="select" type="checkbox" class="select-box" value="all" {{ctx.checked}}="">' : ""; ////tpl.select.resolve(getParams.call(that, { type: "checkbox", value: "all" }));
    ...
    Grid.defaults = {
         ...
         **/
        css: {
            actions: "actions btn-group col-lg-3 col-md-3 col-sm-3 col-xs-12", // must be a unique class name or constellation of class names within the header and footer
            center: "text-center",
            ...
            right: "text-right",
            search: "search form-group col-lg-6 col-md-6 col-sm-6 col-xs-12", // must be a unique class name or constellation of class names within the header and footer
        ...
        templates: {
            ...
            actions: "<div class=\"{{css.actions}}\" style=\"float:left; display:block;\"></div>",
            body: "<tbody></tbody>",
            cell: "<td class=\"{{ctx.css}}\" style=\"{{ctx.style}}\">{{ctx.content}}</td>",
            footer: "<div id=\"{{ctx.id}}\" class=\"{{css.footer}}\"><div class=\"row\"><div class=\"col-sm-6\"><p class=\"{{css.pagination}}\"></p></div><div class=\"col-sm-6 infoBar\"><p class=\"{{css.infos}}\"></p></div></div></div>",
            header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-12 actionBar\" style=\"text-align:center; padding-top:1em;\"><p class=\"{{css.search}}\"></p><p class=\"{{css.actions}}\"></p></div></div></div>",
            // header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-12 actionBar\"><p class=\"{{css.search}} col-lg-3 col-md-3 col-sm-3 col-xs-12\" style=\"float:left\" ></p><p class=\"{{css.actions}} col-lg-3 col-md-3 col-sm-3 col-xs-12 \" style=\"float:left\"></p></div></div></div>", 
            ...
            search: "<div class=\"{{css.search}}\" style=\"float:left;  display:block;\"><div class=\"input-group\"><span class=\"{{css.icon}} input-group-addon {{css.iconSearch}}\"></span> <input type=\"text\" class=\"{{css.searchField}}\" placeholder=\"{{lbl.search}}\" /></div></div>",
